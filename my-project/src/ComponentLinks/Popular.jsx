...
export default function Popular() {
     ...
     return (
          <>
               ...
               {/* Now-Playing */}
               <div>
                    <h1 data-aos="fade-down" className="text-center text-5xl mt-20 font-[600]">Popular Movies</h1>
                    {users.map((items) => {
                         const { id, poster_path, title, release_date  } = items;
                         const url = 'https://image.tmdb.org/t/p/w500/';
                    
                         return (<PopularProps key={id} poster_img={url + poster_path} title={title} releaseDate={release_date} /> );
...
export function PopularProps({ poster_img, title, releaseDate }) {
     return (
          <>
